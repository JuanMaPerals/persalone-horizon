...
final class EmulatorHaloTransport implements HaloTransport {
  ...
  /// EMULATED-only: arms the bridge's constant device-side button reporter,
  /// so each press is reported by the device over Bluetooth like a Halo app.
  Future<void> armButtonReporter() async {
    await _requireBridge().request(<String, Object?>{'op': 'arm_buttons'});
  }

  /// EMULATED-only: presses the emulated hardware button and returns what the
  /// device sent over Bluetooth in response (e.g. `btn:single`).
  Future<List<String>> pressButton(HaloButtonPress press) async {
    final Map<String, Object?> reply = await _requireBridge().request(
        <String, Object?>{'op': 'button', 'kind': press.wire});
    return (reply['device_messages']! as List<Object?>).cast<String>();
  }

  Future<EmulatorFrame> frame({String? png, bool withPng = false}) async {
    final Map<String, Object?> reply = await _requireBridge().request(
        <String, Object?>{'op': 'frame', 'png': png, 'png_base64': withPng});
    final Object? pngBase64 = reply['png_base64'];
    return EmulatorFrame(
      ...
      png: pngBase64 is String ? base64Decode(pngBase64) : null,
    );
  }
...
final class EmulatorFrame {
  ...
  /// The framebuffer as PNG, when requested with `withPng`.
  final Uint8List? png;
}

/// Physical button gestures the official emulator can inject.
enum HaloButtonPress {
  singlePress('single'),
  doublePress('double'),
  longPress('long');

  const HaloButtonPress(this.wire);

  /// Name used by the bridge and in the device's `btn:<wire>` report.
  final String wire;
}
