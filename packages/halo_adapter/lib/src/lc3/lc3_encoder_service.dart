...
class _IsolateConfig {
  ...
  /// PCM input sample rate. When higher than [sampleRateHz], liblc3 downsamples
  /// the PCM input on the fly before encoding (e.g. 24kHz PCM -> 16kHz LC3).
  /// Must be >= [sampleRateHz]; equal to it means no resampling.
...
Future<void> _lc3EncoderIsolate(_IsolateConfig config) async {
  ...
  // PCM input frame is sized by the (possibly higher) PCM input rate
  ...
  try {
    // encoder size is keyed on the PCM (input) sample rate, per lc3.h
    final int encoderStructSize = bindings.lc3_encoder_size(
    ...
    encoder = bindings.lc3_setup_encoder(
      ...
      config.pcmSampleRateHz, // PCM input rate (>= sampleRateHz downsamples)
      encoderMem,
    );
